        ...
        private void DestroyMesh()
        {
            ...
            for (var c = 0; c < CutCascades; c++)
            {
                for (var i = 0; i < _parts.Count; i++)
                {
                    ...
                    subParts.Add(GenerateMesh(_parts[i], plane, true));
                    subParts.Add(GenerateMesh(_parts[i], plane, false));
                }
                _parts = new List<PartMesh>(subParts);
                subParts.Clear();
            }
        }

        private void CreateParts(List<PartMesh> parts, bool canBreak)
        {
            for (var i = 0; i < parts.Count; i++)
            {
                parts[i].MakeGameobject(this, canBreak);
